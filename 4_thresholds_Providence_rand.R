...
globaldistance_providence_target <- feather::read_feather("Data/globaldistance_providence_rand.feather") %>% filter(data_type == "target")
...
target_globalthresholds_l <- target_globalthresholds_providence_base %>%
  left_join(first_instance_base_Providence) %>%
  distinct(Speaker, AOP, gloss1, threshold, .keep_all = TRUE)
