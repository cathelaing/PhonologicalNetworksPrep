...
color_num = 0:max(globalthresholds_lyon$degree) #create a color palette of the same size as the number of vertices.
...
ages <- globalthresholds_lyon %>% filter(data_type == "target") %>% dplyr::select(Speaker, gloss1, age)
...
globaldistance_actual_base <- globaldistance_lyon %>%              # create edges
...
globaldistance_target_base <- globaldistance_lyon %>%             # create edges
...
globaldistance_degree_target <- globalthresholds_lyon %>%         # create nodes base df
  ...
  #left_join(WPs) %>%
  feather::write_feather("Data/globaldistance_degree_target_lyon.feather")

globaldistance_degree_actual <- globalthresholds_lyon %>%        # create nodes base df
  ...
  mutate(AOP = as.numeric(AOP),
         degree = as.numeric(degree)) %>%
  #left_join(WPs) %>%
  feather::write_feather("Data/globaldistance_degree_actual_lyon.feather")
...
save(globalgraphdata_actual, file="Data/globalgraphdata_actual_lyon.Rdata")
save(globalgraphdata_target, file="Data/globalgraphdata_target_lyon.Rdata")
...
#feather::write_feather(globalpathlength_alldata, "Data/globalpathlength_alldataRED_lyon.feather")
#feather::write_feather(globalpathlength_alldata, "Data/globalpathlength_alldata_lyon.feather")
feather::write_feather(globalpathlength, "Data/globalpathlength_lyon.feather")
